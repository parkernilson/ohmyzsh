...
shorten_branch_name() {
  local branch="$1"
  if [[ "$branch" == */* ]]; then
    # If branch contains slashes, show only the last part with ... prefix
    echo ".../${branch##*/}"
  else
    # If no slashes, show the full branch name
    echo "$branch"
  fi
}
...
git_custom_status() {
  ...
  [[ -n "$branch" ]] || return 0
  local short_branch=$(shorten_branch_name "$branch")
  print "%{${fg_bold[yellow]}%}$(work_in_progress)%{$reset_color%}\
${ZSH_THEME_GIT_PROMPT_PREFIX}$(parse_git_dirty)${short_branch}\
